...
def Ceaser_encription ():       
    if entryText.get()!="" or entryKey.get()!="":
        ...
        for i in text:
            ...
            z += 1
        for i in temp1 :
            ziad += alphArr[(i+key+26)%26]
        ceaserEncLb["text"] = "Result: "+ziad
    else:
...
def Vigenere_encription():
    if entryText.get()!="" or entryKey.get()!="":
        ...
        for i in range (len(text)):
            ziad+=matrix[temp1[i]][temp2[i]]
        vigenereEncLb["text"] = "Result: "+ziad
    else:
        handlingLb["text"]="Please Enter Correct Inputs"
        ...
        for i in text:
            for j in range(0,26) :
                if matrix[temp1[y]][j]==i:
                    ziad+=matrix[0][j]
            y+=1   
        vigenereDecLb["text"] = "Result: "+ziad
    else:
        handlingLb["text"]="Please Enter Correct Inputs"
...
def Des_encription():
    if entryText.get()!="" or entryKey.get()!="":
        ...
        ziad = des.encrypt(text).hex()
        print(ziad)
        desEncLb["text"] = "Result: "+str(ziad)
...
def Des_decription():
    if entryText.get()!="" or entryKey.get()!="":
        ...
        key = entryKey.get()
        des = pyDes.des(key, pyDes.ECB, pad=None, padmode=pyDes.PAD_PKCS5)
        ziad = des.decrypt(text)
        desDecLb["text"] = "Result: "+str(ziad)
    else:
        handlingLb["text"]="Please Enter Correct Inputs"
